...
const sectionContainer = document.querySelectorAll(".section-counter");

/* Side Nav */
const sideNavItems = document.querySelectorAll(".nav-icons > a");
const sideNavContainer = document.querySelector(".side-nav");

/* Side Nav */
const mainNavItems = document.querySelectorAll(".nav_link");
...
const headerObserver = new ObserverCreator(
  ...
  (entries) => {
    entries.forEach((entry) => {
      ...
      sideNavContainer.classList.toggle("show", !entry.isIntersecting);
    });
  },
  {
    threshold: 0.45,
  ...
  entries.forEach((entry) => {
    if (entry.isIntersecting) {
      ...
      sideNavItems.forEach((element) => {
        const parentElement = element.closest(".side-nav-items");
        parentElement.classList.toggle(
          "active",
          element.getAttribute("href").includes(href)
        );
      });
    }
  });
...
const sectionObserver = new ObserverCreator(sectionContainer, sectionCallback, {
  threshold: 1,
});
sectionObserver.createObserver();
sectionObserver.observeElement();
